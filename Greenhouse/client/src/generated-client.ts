...
export class SensorClient {
    ...
    getSensorLogs(authorization: string | undefined): Promise<SensorData[]> {
        let url_ = this.baseUrl + "/GetSensorLogs";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetSensorLogs(_response);
        });
    }

    protected processGetSensorLogs(response: Response): Promise<SensorData[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as SensorData[];
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
    ...
    deleteSensorData(authorization: string | undefined): Promise<FileResponse> {
        let url_ = this.baseUrl + "/DeleteSensorData";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processDeleteSensorData(_response);
        });
    }

    protected processDeleteSensorData(response: Response): Promise<FileResponse> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200 || status === 206) {
            const contentDisposition = response.headers ? response.headers.get("content-disposition") : undefined;
...
export class ThresholdsClient {
    ...
    getAll(): Promise<Thresholds> {
        let url_ = this.baseUrl + "/api/sensor/AllThresholds";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processGetAll(response: Response): Promise<Thresholds> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export interface ThresholdUpdateBroadcastDto extends ApplicationBaseDto {
    eventType?: string;
...
export enum StringConstants {
    ...
    ThresholdUpdateBroadcastDto = "ThresholdUpdateBroadcastDto",
    ...
    Dashboard = "Dashboard",
    ThresholdsHistory = "ThresholdsHistory",
    Device = "Device",
